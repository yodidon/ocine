...
def showMovies(sSearch = ''):
    ...
    if sSearch:
      ...
      sPattern = '<div class="result-item">.*?<a href="([^"]+)"><img src="([^"]+)".*?<div class="title"><a.*?>([^"]+)</a.*?class="year">([^"]+)</span>.*?<div class="contenido"><p>([^"]+)</p>'
        ...
        sPattern = 'article id="post-[\d].+?<img src="([^"]+).+?alt="([^"]+).+?quality">(\w.).(\w.+?)<.+?href="([^"]+).+?class="imdb".+?<span>([^<]+).+?class="texto">([^<]+)'
    ...
    if (aResult[0] == True):
        ...
        for aEntry in aResult[1]:
            ...
            sDisplayTitle = ('%s (%s %s) [%s]') % (sTitle, sYear,sQual ,sLang)
...
def showLinks():
    ...
    sPattern = "class='dooplay_player_option' data-type='([^']+)' data-post='([^']+)' data-nume='([^']+)'.+?'server'>([^.]+)"
    aResult = oParser.parse(sHtmlContent, sPattern)
...
def showHosters():
    ...
    oRequest.addHeaderEntry('User-Agent', 'Mozilla/5.0 (Windows NT 10.0; ) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.129 Safari/537.36')
